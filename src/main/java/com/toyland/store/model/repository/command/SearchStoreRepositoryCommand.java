  ...
  public OrderSpecifier[] orderSpecifiers() {
    ...
    for (int i = 0; i < sort.size(); i++) {
      ...
      if (sortTypeMap.containsKey(upperParam)) {
        ...
        if(sortType == StoreSortType.CREATED_AT) createdAtCheck = true;
        if(sortType == StoreSortType.UPDATED_AT) updatedAtCheck = true;
      }
    ...
    if(!createdAtCheck) orders.add(StoreSortType.CREATED_AT.getSpecifier(Direction.ASC));
    if(!updatedAtCheck) orders.add(StoreSortType.UPDATED_AT.getSpecifier(Direction.ASC));
